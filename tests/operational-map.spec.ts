...
for(const width of [390,1440])test(`broad operational map and small solar evidence ${width}`,async({page})=>{
 await page.setViewportSize({width,height:900});await page.goto('/');await page.getByRole('button',{name:'Generation',exact:true}).click();await page.locator('.map-filter-drawer > summary').click();await page.locator('.map-results-drawer > summary').click();
 ...
 await page.getByLabel('Minimum asset capacity').selectOption('0');await page.getByRole('button',{name:'Show map',exact:true}).click();await page.getByRole('button',{name:'Inspect Fen Farm Solar Park generation',exact:true}).click();
...
test('catalogue fetch failure retains curated map with explicit coverage warning',async({page})=>{await page.route('**/data/operational-assets.json',r=>r.fulfill({status:503,body:'Unavailable'}));await page.goto('/');await page.getByRole('button',{name:'Generation',exact:true}).click();await page.locator('.map-filter-drawer > summary').click();await page.locator('.map-results-drawer > summary').click();await expect(page.getByText('Wider catalogue unavailable; retained curated entries shown.')).toBeVisible();await expect(page.locator('.atlas-access-list[data-layer=generation] button')).toHaveCount(47)});
