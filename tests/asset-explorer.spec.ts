...
for(const width of [390,1440])test(`asset explorer navigation and filters ${width}`,async({page})=>{
 ...
 await page.getByRole('button',{name:'Show existing 47-site catalogue'}).click();const svg=page.locator('svg.asset-navigable');const start=await svg.getAttribute('viewBox');
 ...
 await page.getByLabel('Asset geography').selectOption('Wales');await expect(page.locator('.atlas-access-list[data-layer=generation]')).toContainText('Pembroke');await expect(page.locator('.atlas-access-list[data-layer=generation]')).not.toContainText('Drax');
