...
for(const width of [390,1440])test(`asset explorer navigation and filters ${width}`,async({page})=>{
 ...
 await expect(page.locator('.atlas-access-list[data-layer=generation] button')).toHaveCount(60);await page.getByRole('button',{name:'Show map',exact:true}).click();const svg=page.locator('svg.asset-navigable');const start=await svg.getAttribute('viewBox');
 await page.getByRole('button',{name:'Zoom in on generation assets'}).click();expect(await svg.getAttribute('viewBox')).not.toBe(start);
 if(width>600)await page.getByRole('button',{name:'Pan east',exact:true}).click();await page.getByRole('button',{name:'Reset GB view'}).click();await expect(svg).toHaveAttribute('viewBox',start!);
 await page.locator('.map-filter-drawer > summary').click();await page.getByLabel('Asset geography').selectOption('Wales');await page.getByRole('searchbox',{name:'Find a generation site'}).fill('Pembroke');await expect(page.locator('.atlas-access-list[data-layer=generation]')).toContainText('Pembroke');await expect(page.locator('.atlas-access-list[data-layer=generation]')).not.toContainText('Drax');
