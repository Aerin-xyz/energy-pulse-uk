...
test('nearby clusters expose individual sites; unmapped sites never show zero',async({page})=>{
 await page.goto('/');await page.getByRole('button',{name:'Generation',exact:true}).click();await expect(page.locator('.atlas-access-list[data-layer=generation] button')).toHaveCount(60);
