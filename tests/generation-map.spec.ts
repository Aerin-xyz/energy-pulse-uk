...
for(const width of [390,1440])test(`generation layer ${width}px: filters, evidence and preserved cables`,async({page})=>{
 ...
 await page.getByRole('button',{name:'Show map',exact:true}).click();await page.getByRole('button',{name:'Inspect Drax generation',exact:true}).click();
 ...
 await page.getByRole('searchbox',{name:'Find a generation site'}).fill('Hornsea One');await page.locator('.map-filter-drawer > summary').click();await page.getByRole('button',{name:'Offshore wind',exact:true}).click();await page.getByRole('button',{name:'Show map',exact:true}).click();await expect(page.locator('.generation-layer [role=button]')).toHaveCount(1);
...
test('unavailable generation never looks like measured zero',async({page})=>{
 await page.route('**/data/generation-assets.json',r=>r.fulfill({json:{checkedAt:null,points:[]}}));await page.goto('/');await page.getByRole('button',{name:'Generation',exact:true}).click();await page.locator('.map-filter-drawer > summary').click();await page.locator('.map-results-drawer > summary').click();await page.getByRole('searchbox',{name:'Find a generation site'}).fill('Drax');await page.getByRole('button',{name:'Inspect Drax generation'}).click();await expect(page.getByRole('region',{name:'Selected map evidence'}).getByText('Output unavailable',{exact:true})).toBeVisible();await expect(page.getByRole('region',{name:'Selected map evidence'})).toContainText('0 / 4');
});

test('nearby clusters expose individual sites; unmapped sites never show zero',async({page})=>{
 await page.goto('/');await page.getByRole('button',{name:'Generation',exact:true}).click();await page.locator('.map-filter-drawer > summary').click();await page.locator('.map-results-drawer > summary').click();await expect(page.locator('.atlas-access-list[data-layer=generation] button')).toHaveCount(60);
