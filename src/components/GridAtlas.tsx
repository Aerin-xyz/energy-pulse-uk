...
export function GridAtlas({
  ...
  return (
    ...
    >
      ...
      </div><p className="asset-legend">Solid rings: notified or dated output—see label · dotted rings: no linked output. Drag to explore; zoom separates nearby sites.</p></>}
<button className="map-filter-close" onClick={e=>e.currentTarget.closest("details")?.removeAttribute("open")}>Show map</button></div></details></div>}
      {mode==='generation'&&<div className="notification-map-summary"><button onClick={()=>{setMinCapacity(100);setPilotOnly(false);setAssetFilter('All');setAssetCountry('All GB');setAssetAvailability('All data');setAssetSearch('');setSelected(null)}}>Explore output above 100 MW</button><span>Notified plans—not measured production. Select a site for its current half-hour and last measured reading.</span></div>}
