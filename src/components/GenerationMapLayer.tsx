...
export function matchingAssets(filter:string,search:string){
 const q=search.trim().toLocaleLowerCase();
 return assets.filter(a=>(filter==='All'||a.type===filter)&&(!q||`${a.name} ${a.type} ${a.country}`.toLocaleLowerCase().includes(q)));
}
export function GenerationMapLayer({filter,search,selected,onSelect,snapshot}:{filter:string;search:string;selected:string|null;onSelect:(id:string)=>void;snapshot:AssetSnapshot}){
 const visible=matchingAssets(filter,search);
 ...
 const a=members.find(a=>a.id===selected)||members[0];const r=snapshot.points.at(-1)?.values[a.id];const mw=r?.mw;const known=typeof mw==='number';
...
export function AssetEvidence({id,snapshot,error,onSelect}:{id:string;snapshot:AssetSnapshot;error:boolean;onSelect:(id:string)=>void}){
 ...
 <p className="asset-small">Registry checked 15 September 2026. {snapshot.checkedAt ? `Feed checked ${assetTime(snapshot.checkedAt)} UK. Updated with the daily site refresh.` : "Feed check time unavailable."}</p><div className="asset-sources"><a href={asset.capacitySource} target="_blank" rel="noreferrer">DESNZ asset register ↗</a><a href="https://bmrs.elexon.co.uk/actual-generation-output-per-generation-unit" target="_blank" rel="noreferrer">Elexon metered data ↗</a><a href={asset.geographySource} target="_blank" rel="noreferrer">Site geography ↗</a></div>
 </div>
}
